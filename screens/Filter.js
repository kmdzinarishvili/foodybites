...
const Category = ({ country, id, pressedId, setPressedId }) => {
	...
	return (
		<Pressable onPress={() => setPressedId(id)}>
			<View
				style={{
					width: 300 * w,
					height: 133 * h,
			...
			>
				{pressed ? (
					<LinearGradient
						...
						colors={gradient}
					>
						<Text style={[styles.jSemi, { fontSize: 50 * w, color: '#FFF' }]}>
							{country}
						</Text>
					</LinearGradient>
				) : (
					<Text
						style={[
							styles.jSemi,
							{ fontSize: 50 * w, color: '#rgba(62, 63, 104, 0.5)' },
						]}
					>
						{country}
					</Text>
...
const Filter = () => {
	...
	return (
		...
		>
			<FilterTitle
				style={{ marginTop: 166 * h, marginBottom: 83 * h }}
				text="Select Category"
			/>
			<CategoryButtons countries={countries} ref={categoryRef} />

			<FilterTitle
				style={{ marginTop: 127 * h, marginBottom: 83 * h }}
				text="Distance"
			/>
			...
			<BlueFooter
				text={['Reset', 'Apply']}
				action={[reset, () => navigation.navigate('Home Search')]}
			/>
		</View>
	);
};
